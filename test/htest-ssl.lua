...
local req_per_sec = 0
local req_per_sec_old = 0

--	utils.spawn(function ()
--		while true do
--			utils.sleep(1)
--			local req = req_per_sec - req_per_sec_old
--			req_per_sec_old = req_per_sec
--			print(req)
--		end
--	end)
...
Hathaway({host="*", port=port,
	wrapHTTP=function (handleHTTP, httpServer, client)
		if client then
			local newc, err = tlsconf:ssl_wrap_socket(client)
			if newc then
				handleHTTP(httpServer, newc)
			end
		end
	end})
